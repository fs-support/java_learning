...
public class GameManager {
	...
	public void Start() {
		...
		for (int i = 0; i < 3; i++) {
			partyAgiSort.AppendPlayer(partyA.GetMembers().get(i));
		}
		for (int i = 0; i < 3; i++) {
			partyAgiSort.AppendPlayer(partyB.GetMembers().get(i));
		}
		...
		while (turnNumber <= 20) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (attackPlayer.GetHP() > 0) {
					...
					if (partyA.GetMembers().contains(attackPlayer)) {
						attackPlayer.Attack(partyA.GetMembers(),
								partyB.GetMembers());
					} else {
						attackPlayer.Attack(partyB.GetMembers(),
								partyA.GetMembers());
					}
					...
					partyAHP = partyA.GetMembers().get(0).GetHP()
							+ partyA.GetMembers().get(1).GetHP()
							+ partyA.GetMembers().get(2).GetHP();
					partyBHP = partyB.GetMembers().get(0).GetHP()
							+ partyB.GetMembers().get(1).GetHP()
							+ partyB.GetMembers().get(2).GetHP();
					if (partyAHP <= 0) {
						System.out.println("\nパーティーAは全滅した...");
						break;
					}
			...
			// 各パーティー全員のHPが0なら戦闘終了
			if (partyA.GetMembers().get(0).GetHP()
					+ partyA.GetMembers().get(1).GetHP()
					+ partyA.GetMembers().get(2).GetHP() <= 0) {
				break;
			}
			if (partyB.GetMembers().get(0).GetHP()
					+ partyB.GetMembers().get(1).GetHP()
					+ partyB.GetMembers().get(2).GetHP() <= 0) {
				break;
			}

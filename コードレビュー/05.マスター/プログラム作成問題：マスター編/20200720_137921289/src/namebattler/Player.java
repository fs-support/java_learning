...
public class Player {
	...
	public void PrintStatus() {
		...
		System.out
				.printf("%s (HP=%3d : MP=%3d : STR=%3d : DEF=%3d : LUK=%3d : AGI=%3d)\n",
						this.GetName(), this.GetHP(), this.GetMP(),
						this.GetSTR(), this.GetDEF(), this.GetLUK(),
						this.GetAGI());
	}
	...
	protected void RemoveParty(Player defenceplayer) {
		...
		if (defenceplayer.hp <= 0) {
			System.out.println(defenceplayer.name + "は力尽きた...");
			System.out.println("");
			...
			// 倒れたプレイヤーを死亡リストに追加
			if (defenceplayer.partynum == 1) {
				GameManager.deadparty.AppendPlayer(defenceplayer);
			} else {
				GameManager.deadparty2.AppendPlayer(defenceplayer);
			}
		}

	}
	
	//全体攻撃のダメージを計算（Wizard,Braveのみ）
	protected void AllfinalDamege(Player atacker){
		atacker.finalatack -= 1;
		
		switch(atacker.job){
		case 2:
			if(atacker.partynum == 1){
				for(int i = 0;i < GameManager.makeparty2.GetMembers().size() ; i++){
					GameManager.makeparty2.GetMembers().get(i).hp -= 100;
					}
			}else{
				for(int i = 0;i < GameManager.makeparty.GetMembers().size() ; i++){
					GameManager.makeparty.GetMembers().get(i).hp -= 100;
					}
			}
			break;
		case 4:
			if(atacker.partynum == 1){
				for(int i = 0;i < GameManager.makeparty2.GetMembers().size() ; i++){
					GameManager.makeparty2.GetMembers().get(i).hp -= 150;
					}
			}else{
				for(int i = 0;i < GameManager.makeparty.GetMembers().size() ; i++){
					GameManager.makeparty.GetMembers().get(i).hp -= 150;
					}
			}
			break;
	...
	//単体に強力攻撃Fighterのみ
	protected void SinglefinalDamege(Player atacker,Player defenceplayer){
		atacker.finalatack -= 1;
		
		defenceplayer.hp -= 100;
	}
	
	//パーティ全体を回復させるPriestのみ
	protected void AllPartyHeel(Player atacker){
		if(atacker.partynum == 1){
			for(int i = 0;i < GameManager.makeparty.GetMembers().size() ; i++){
				GameManager.makeparty.GetMembers().get(i).hp += 100;
				}

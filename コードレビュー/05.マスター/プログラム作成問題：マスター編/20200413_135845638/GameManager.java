	...
	void start(){
		...
		while (turnNumber<=3) {
			...
			for(int i = 0; i<allMembers.GetMembers().size(); i++){
				Player attacker = allMembers.GetMembers().get(i);

				// どちらかのパーティーが全滅していたらループ(for)を抜ける
				...
				if(defender.GetHP() <= 0) {
					...
					for(int j = 0; j<allMembers.GetMembers().size(); j++){
						if(allMembers.GetMembers().get(j)==defender){
							allMembers.RemovePlayer(allMembers.GetMembers().get(j));
						}
					}
			...
			// どちらかのパーティーが全滅していたらループ(while)を抜ける
			if((party1.GetMembers().size()==0) || (party2.GetMembers().size()==0)){
				break;
			}
